    ...
    public class PlotsController : Controller
    {
        ...
        public void UpdatePlotInstallmentStatus(List<Sp_Get_PlotInstallments_Result> inst, List<Sp_Get_ReceivedAmounts_Result> Receipts, List<Discount> Dis, long Plotid)
        {
            ...
            if (duedate != null)
            {
                ...
                db.Test_updatebalance(remamt, res1.Where(x => x.Installment_Type != "10" && x.Installment_Type != "0" && x.Installment_Type != "11").Sum(x => x.Amount), ReceivedAmount, Dis.Sum(x => x.Discount_Amount), Plotid, Modules.PlotManagement.ToString(), id.Count(), duedate.DueDate, voucher.Sum(x => x.Amount));
